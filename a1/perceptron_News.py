...
if __name__ == "__main__":
    ...
    model = PerceptronModel(k = k, max_iters = 20)
    model.train(train_data)

    # Predict on the development set. 
    dev_accuracy = evaluate(model,
                            dev_data,
                            os.path.join("results", "perceptron_" + data_type + "_dev_predictions.csv"), labels_dict = labels_dict)
    ...
    evaluate(model,
             test_data,
             os.path.join("results", "perceptron_" + data_type + "_test_predictions.csv"), labels_dict = labels_dict)
